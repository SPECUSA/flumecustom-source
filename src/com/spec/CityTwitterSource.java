 ...
 * updates.
 */
public class CityTwitterSource extends AbstractSource
    implements EventDrivenSource, Configurable {
  
  private static final Logger logger =
      LoggerFactory.getLogger(CityTwitterSource.class);
  ...
  public void configure(Context context) {
    ...
    String citystring=context.getString(TwitterSourceConstants.CITIES_KEY, "");
  ...
  public void start() {
    ...
    StatusListener listener = new StatusListener() {
      ...
      public void onStatus(Status status) {
        ...
        flag=false;
        for(int i=0;i<cities.length;i++){
            if(status.getUser().getLocation().toLowerCase().contains(cities[i].toLowerCase())) flag=true;
        }
        if(flag){
        ...
        Event event = EventBuilder.withBody(
            DataObjectFactory.getRawJSON(status.getUser()).getBytes(), headers);
